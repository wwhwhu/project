...
def collect_data():
    # 读取data文件夹下mat文件
    data = scipy.io.loadmat("data/qingtian.mat")
    print("数据采集完成", '\n晴天：\n', data.keys())
    print("数据晴天：",  data['__function_workspace__'].shape, data['__function_workspace__'].dtype, '内容：\n', data['__function_workspace__'])
    return data['__function_workspace__'][0]


# 低通滤波后下采样
def low_pass_downsample(mysignal):
    # 低通滤波
    # 采样频率
    fs = 1000
    # 低通滤波器
    b, a = signal.butter(8, 0.1)
    signal_low = signal.filtfilt(b, a, mysignal)
    # 下采样
    signal_downsampled = signal_low[::10]
    print("低通滤波后下采样：", signal_downsampled.shape, signal_downsampled.dtype, "数据内容：", signal_downsampled)
...
def main():
    ...
    chunks = np.array_split(noise_removed, 15)  # 假设我们将数据分为10块进行并行处理
    with Pool(15) as p:
